...
def test_add_and_read_one_airplane(saver_with_temp_dir):
    plane = Airplane(callsign="TEST123", origin_country="TestLand", velocity=0, altitude=0, on_ground=True)
    ...
    assert data[0]["on_ground"] is True


def test_add_multiple_airplanes_order(saver_with_temp_dir):
    planes = [
        Airplane(callsign=f"MULTI{i}", origin_country="Land", velocity=0, altitude=0, on_ground=True) for i in range(3)
    ]
    for p in planes:
...
def test_get_airplanes_filter_by_country(saver_with_temp_dir):
    planes = [
        Airplane(callsign="1", origin_country="USA", velocity=0, altitude=0, on_ground=True),
        Airplane(callsign="2", origin_country="USA", velocity=0, altitude=0, on_ground=True),
        Airplane(callsign="3", origin_country="Canada", velocity=0, altitude=0, on_ground=True),
    ]
    for p in planes:
        saver_with_temp_dir.add_airplane(p)
...
def test_delete_airplane_success(saver_with_temp_dir):
    fly1 = Airplane(callsign="1", origin_country="USA", velocity=0, altitude=0, on_ground=True)
    fly2 = Airplane(callsign="2", origin_country="USA", velocity=0, altitude=0, on_ground=True)
